...
fn period_summary_failure_rate_is_correct() {
    ...
    assert!(
        (summary.failure_rate_pct - expected).abs() < 0.01,
        "Expected failure rate ~{expected}%, got {:.2}%",
...
fn failure_trend_detects_improvement() {
    ...
    let jan_txs: Vec<Transaction> = (0..5)
        .map(|i| {
            make_tx(
                &format!("ok-{i}"),
                "acc-a",
    ...
    let feb_txs: Vec<Transaction> = (0..9)
        .map(|i| {
            make_tx(
                &format!("ok2-{i}"),
                "acc-a",
